...
from taskport.errors import ConfigurationError, ExecutionNotFound, UnsupportedCapability
from taskport.execution import ExecutionId, new_execution_id
from taskport.hooks import BaseHook, HookChain, LoggingHook
from taskport.plugins import build_backend, registered_backends, resolve_factory
from taskport.ports import BaseBackend, accepts, supports
from taskport.specs import ExecutionSpec, InlineSpec, JobSpec, TaskSpec
from taskport.tracking import ExternalIdIndex, has_prefix, is_digits
...
class TestPlugins:
    ...
    def test_the_builtin_backends_are_always_discoverable(self) -> None:
        """These ship with taskport, so they are available with nothing installed."""
    ...
    def test_installed_adapters_are_discovered_through_entry_points(self) -> None:
        ...
        situation CI's isolation job creates on purpose, to prove `taskport`
    ...
    def test_an_import_string_is_accepted(self) -> None:
        factory = resolve_factory("taskport.backends.inline:InlineExecutionBackend")
