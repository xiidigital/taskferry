...
Taskport's normal tests check that Taskport works. These check the shape a
...
flowchart LR
    ...
    AD["its taskport adapter<br/>structural Protocols"]
    TP["taskport"]
...
* that adapter declares Taskport's runtime as a **structural `Protocol`**, so it
  imports, type-checks and unit-tests with Taskport absent;
* the application injects a configured runtime.

Nothing here imports a consumer. These are assertions about Taskport's own
...
are all source-compatible *within* Taskport and all break an embedder silently.
"""
...
# not by importing Taskport's own classes.
# --------------------------------------------------------------------------- #
@runtime_checkable
...
class TestTheRuntimeSurface:
    def test_the_runtime_satisfies_a_structural_protocol(self, runtime: Taskport) -> None:
        """An embedder declares a Protocol so it can be imported without Taskport."""
        assert isinstance(runtime, EmbedderRuntime)

    def test_all_three_facades_exist(self, runtime: Taskport) -> None:
    ...
    def test_lookup_signatures_are_stable(
        self, runtime: Taskport, method: str, required: list[str]
    ) -> None:
    ...
    def test_the_job_facade_accepts_the_portable_vocabulary(self, runtime: Taskport) -> None:
...
class TestTheHandleSurface:
    def test_the_handle_satisfies_a_structural_protocol(self, runtime: Taskport) -> None:
    ...
    def test_the_id_is_a_plain_string_a_consumer_can_store(self, runtime: Taskport) -> None:
    ...
    def test_the_backend_name_is_readable_for_a_later_lookup(self, runtime: Taskport) -> None:
...
class TestCapabilityNegotiation:
    ...
    def test_capabilities_are_readable_before_acting(self, runtime: Taskport) -> None:
    ...
    def test_capabilities_are_readable_from_the_runtime_too(self, runtime: Taskport) -> None:
    ...
    def test_an_unsupported_operation_raises_a_taskport_error(self, runtime: Taskport) -> None:
        """Everything a consumer can catch is one type.

        An adapter is expected to translate this into its own hierarchy — the
        same discipline Taskport applies to provider SDKs. This test pins the
        ...
        with pytest.raises(UnsupportedCapability) as caught:
            handle.cancel()
        assert isinstance(caught.value, TaskportError), (
            "every catchable Taskport error must subclass TaskportError, so a "
            "consumer can translate the whole family with one except clause"
        )

    def test_every_public_error_subclasses_taskport_error(self) -> None:
        """One `except TaskportError` has to be enough at an integration boundary."""
        import taskport

        errors = [
            getattr(taskport, name)
            for name in taskport.__all__
            if isinstance(getattr(taskport, name), type)
            and issubclass(getattr(taskport, name), BaseException)
        ]
        assert errors, "the public API should export its exception types"
        for error in errors:
            assert issubclass(error, TaskportError), (
                f"{error.__name__} is exported but is not a TaskportError; a consumer "
...
class TestTheDispatcherPattern:
    ...
    cross a process boundary. Taskport refuses such a callable at submit time
    rather than failing on the worker.

    The way through is one importable dispatcher plus the real identity as data,
    which is exactly what `taskport_django` does for Django's Task objects. This
    pins that the pattern works, so an embedder can rely on it.
    """

    def test_a_stateful_callable_is_refused_early(self, runtime: Taskport) -> None:
    ...
    def test_a_callable_instance_is_refused_early(self, runtime: Taskport) -> None:
    ...
    def test_a_classmethod_is_still_accepted(self, runtime: Taskport) -> None:
    ...
    def test_the_same_callable_is_fine_inline(self, runtime: Taskport) -> None:
        """Which is why an embedder can always fall back to Inline."""
        configured = _Processor(factor=3)
        assert runtime.inline.run(configured.run, 14) == 42

    def test_a_dispatcher_carries_the_identity_as_data(self, runtime: Taskport) -> None:
    ...
    def test_the_dispatcher_keeps_the_real_identity_visible(self, runtime: Taskport) -> None:
...
class TestIndependenceIsStructural:
    def test_taskport_needs_no_knowledge_of_its_embedders(self) -> None:
        """The dependency arrow points one way, and nothing here breaks that."""
        import taskport

        assert not hasattr(taskport, "dratl")
        assert taskport.__all__, "the public API is the whole contract surface"
    ...
    def test_a_runtime_is_injectable(self) -> None:
        ...
        rt = Taskport(config=Taskport.local().config, backends={"inline": injected})
        try:
            assert rt.backend("inline") is injected
        finally:
            rt.close()

    def test_an_execution_snapshot_is_a_plain_value(self, runtime: Taskport) -> None:
    ...
    def test_a_handle_is_not_meant_to_be_pickled(self, runtime: Taskport) -> None:
