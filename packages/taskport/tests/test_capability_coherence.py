...
`taskport.Capability` is the **execution** vocabulary: what a backend can do with
...
```mermaid
flowchart BT
    EXEC["taskport.Capability<br/>submit · state · cancel · gpu · ..."]
    EV["EventCapability<br/>fanout · replay · retention"]
    SCH["ScheduleCapability<br/>cron · timezone · pause"]
    BASE["taskport.core.Capability<br/>the shared StrEnum base"]
...
from taskport.core.capabilities import Capability as BaseCapability
from taskport.core.capabilities import CapabilitySet
from taskport.core.errors import UnsupportedCapabilityError
...
class TestOneSharedFoundation:
    ...
    def test_every_vocabulary_shares_the_base(self, name: str) -> None:
        ...
        assert issubclass(vocabulary, BaseCapability), (
            f"{vocabulary.__name__} must subclass taskport.core.Capability so that "
...
class TestTheExecutionVocabularyIsShared:
    ...
    def test_the_execution_vocabulary_covers_every_spec_requirement(self) -> None:
        ...
        from taskport import JobSpec, Resources, TaskSpec, TimeoutPolicy
