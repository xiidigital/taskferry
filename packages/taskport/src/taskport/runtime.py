"""The Taskport runtime — one object an application holds and passes around.
...
``import taskport`` never reaches a provider SDK.
...
class Taskport:
    ...
    Args:
        config: Where backends, routes and defaults come from. Defaults to
            :meth:`TaskportConfig.from_env`, so a Twelve-Factor deployment needs
            no code at all.
        ...
        hooks: Lifecycle observers. See :mod:`taskport.hooks`.
    """

    def __init__(
        self,
        *,
        config: TaskportConfig | None = None,
        ...
        self._config = config if config is not None else TaskportConfig.from_env()
    ...
    # -- constructors ------------------------------------------------------- #
    @classmethod
    def local(cls, **kwargs: Any) -> Taskport:
        ...
        """
        kwargs.setdefault("config", TaskportConfig.local())
        return cls(**kwargs)

    @classmethod
    def from_env(cls, environ: Mapping[str, str] | None = None, **kwargs: Any) -> Taskport:
        """Build from ``TASKPORT_*`` environment variables. See :meth:`TaskportConfig.from_env`."""
        kwargs.setdefault("config", TaskportConfig.from_env(environ))
        return cls(**kwargs)

    @classmethod
    def from_mapping(cls, data: Mapping[str, Any], **kwargs: Any) -> Taskport:
        """Build from a plain mapping (TOML, YAML, Django settings, a literal dict)."""
        kwargs.setdefault("config", TaskportConfig.from_mapping(data))
        return cls(**kwargs)
    ...
    def get(
        ...
        boundary that memory is gone, so pass ``backend=`` — Taskport does not
    ...
    def __repr__(self) -> str:
        return f"<Taskport backends={list(self.backend_names())}>"
...
class _Facade:
    ...
    def __init__(self, runtime: Taskport) -> None:
