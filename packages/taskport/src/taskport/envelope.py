...
flowchart LR
    SPEC["TaskSpec"]
    ENV["envelope<br/>{taskport, task, args, kwargs, correlation, retry}"]
...
def read_envelope(payload: Any) -> Envelope:
    ...
    if not isinstance(payload, dict):
        raise SerializationError(
            f"a taskport envelope must be a JSON object, got {type(payload).__name__}"
        )
    version = payload.get("taskport")
    if version != ENVELOPE_VERSION:
        raise SerializationError(
            f"unsupported taskport envelope version {version!r} "
            f"(this worker speaks {ENVELOPE_VERSION!r}); upgrade the worker or the producer"
        )
    if not payload.get("task"):
        raise SerializationError("taskport envelope has no 'task'")
...
def execute_envelope(payload: Any, *, registry: Any = None) -> Any:
    ...
    Args:
        ...
        registry: A :class:`~taskport.functions.FunctionRegistry`. **Give it an
