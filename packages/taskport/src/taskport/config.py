...
internal model is :class:`TaskportConfig`: typed, immutable, validated once at
...
framework's registry. ``taskport_django.config_from_settings()`` builds a
``TaskportConfig`` from ``settings.TASKPORT`` and hands it over — the dependency
...
ENV_PREFIX = "TASKPORT_"

DEFAULT_BACKENDS: Mapping[str, str] = MappingProxyType(
    {
        "inline": "taskport.backends.inline:InlineExecutionBackend",
        "thread": "taskport.backends.thread:ThreadTaskBackend",
        "process": "taskport.backends.process:ProcessTaskBackend",
        "subprocess": "taskport.backends.subprocess:SubprocessJobBackend",
    }
)
...
class BackendConfig:
    ...
    Attributes:
        factory: Either a plugin name registered under the ``taskport.backends``
...
class TaskportConfig:
    ...
    Attributes:
        backends: Named backend definitions. Instantiated lazily, so configuring
            ...
            only enqueues tasks.
        routes: Ordered routing rules. See :class:`~taskport.router.Route`.
    ...
    def router(self) -> Router:
        """Build the :class:`~taskport.router.Router` this configuration describes."""
        return Router(self.routes, defaults=self.defaults)

    def evolve(self, **changes: Any) -> TaskportConfig:
        """Return a modified copy. Configs are immutable."""
        return replace(self, **changes)

    def with_backend(self, name: str, config: BackendConfig) -> TaskportConfig:
        """Return a copy with one more backend defined."""
        return self.evolve(backends={**self.backends, name: config})

    # -- loaders ------------------------------------------------------------ #
    @classmethod
    def local(cls) -> TaskportConfig:
        ...
        :meth:`taskport.Taskport.local` uses, and it is a genuinely useful
    ...
    @classmethod
    def from_mapping(cls, data: Mapping[str, Any]) -> TaskportConfig:
    ...
    @classmethod
    def from_env(cls, environ: Mapping[str, str] | None = None) -> TaskportConfig:
        """Build from ``TASKPORT_*`` environment variables.
        ...
        ``TASKPORT_DEFAULT_INLINE`` · ``TASKPORT_DEFAULT_TASK`` ·
        ``TASKPORT_DEFAULT_JOB`` · ``TASKPORT_ALLOW_IMPORT`` ·
        ``TASKPORT_ALLOWED_MODULES`` (comma-separated).

        With nothing set at all this returns :meth:`local`, so a container that
        forgot to configure Taskport still starts and still runs work — locally,
