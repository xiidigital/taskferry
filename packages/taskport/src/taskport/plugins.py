...
An adapter distribution advertises its backends in its own ``pyproject.toml``::
    ...
    procrastinate = "taskport_procrastinate:make_backend"

which lets a deployment write ``{"factory": "procrastinate", ...}`` in config and
have it work the moment the package is installed — without `taskport` importing,
knowing about, or depending on that package.

```mermaid
flowchart BT
    PRO["taskport-procrastinate<br/>entry point: procrastinate"]
    CR["taskport-cloudrun<br/>entry point: cloudrun"]
    CT["taskport-cloudtasks<br/>entry point: cloudtasks"]
    EP["taskport.backends<br/>entry-point group"]
    TP["taskport<br/>load_backend()"]
...
ENTRY_POINT_GROUP = "taskport.backends"
...
def resolve_factory(spec: str) -> BackendFactory:
    ...
    2. an installed ``taskport.backends`` entry point;
    ...
    raise ConfigurationError(
        ...
        f"taskport.register_backend(), or use a 'module:callable' import string "
        f"(available: {known})"
    )
...
def build_backend(factory_spec: str, options: Mapping[str, Any]) -> ExecutionBackend:
    ...
    :class:`~taskport.core.errors.ConfigurationError`, because "you passed an
