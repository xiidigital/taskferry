...
Run Jobs can allocate a GPU; a thread pool cannot. Taskport refuses to paper over
this: every backend advertises an immutable :class:`~taskport.core.CapabilitySet`
and any operation requiring a capability it does not advertise raises
:class:`~taskport.errors.UnsupportedCapability` instead of being silently faked
or approximated.
...
:class:`taskport.core.Capability`; the values here are the portable vocabulary.
"""

from __future__ import annotations

from taskport.core.capabilities import Capability as _CapabilityBase
from taskport.core.capabilities import CapabilitySet


class Capability(_CapabilityBase):
    """The portable capability vocabulary shared by every Taskport backend."""

    # -- submission --------------------------------------------------------- #
    SUBMIT = "submit"
    """Accept a spec and return an :class:`~taskport.execution.Execution`.
    ...
    RETRY = "retry"
    """Apply a :class:`~taskport.retry.RetryPolicy` natively."""
