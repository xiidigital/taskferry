"""The async surface: `AsyncTaskport`, with the same method names.
...
`AsyncTaskport` mirrors :class:`~taskport.runtime.Taskport` method for method,
...
flowchart TD
    ...
    ART["AsyncTaskport"]
    RT["Taskport<br/>routing · config · backends"]
    BE["backend.asubmit()"]
...
An `AsyncTaskport` **wraps** a sync runtime rather than duplicating it. Routing,
...
aio = AsyncTaskport(runtime)          # same backends, same everything
...
:class:`~taskport.ports.BaseBackend`). That is a real async path — the caller's
...
from .config import TaskportConfig
...
from .runtime import Taskport, _as_backend_options, _as_timedelta, _as_timeout
from .specs import AnySpec, BackendOptions, InlineSpec, JobSpec, Resources, TaskSpec


class AsyncExecutionHandle:
    """The async twin of :class:`~taskport.handle.ExecutionHandle`.
    ...
    async def result(self, timeout: float | None = None) -> ExecutionResult:
        """Return the outcome, raising :class:`~taskport.errors.ExecutionError` on failure."""
...
class AsyncTaskport:
    ...
    Args:
        runtime: An existing :class:`~taskport.runtime.Taskport` to share. When
    ...
    def __init__(self, runtime: Taskport | None = None, /, **kwargs: Any) -> None:
        self._runtime = runtime if runtime is not None else Taskport(**kwargs)
    ...
    # -- constructors ---------------------------------------------------------- #
    @classmethod
    def local(cls, **kwargs: Any) -> AsyncTaskport:
        """A runtime that needs no infrastructure. See :meth:`Taskport.local`."""
        return cls(Taskport.local(**kwargs))

    @classmethod
    def from_env(cls, environ: Mapping[str, str] | None = None, **kwargs: Any) -> AsyncTaskport:
        return cls(Taskport.from_env(environ, **kwargs))

    @classmethod
    def from_mapping(cls, data: Mapping[str, Any], **kwargs: Any) -> AsyncTaskport:
        return cls(Taskport.from_mapping(data, **kwargs))

    # -- introspection (shared, synchronous — no I/O) --------------------------- #
    @property
    def sync(self) -> Taskport:
        """The underlying sync runtime. Use it from sync code in the same process."""
        return self._runtime

    @property
    def config(self) -> TaskportConfig:
    ...
    def __repr__(self) -> str:
        return f"<AsyncTaskport backends={list(self.backend_names())}>"
...
class _AsyncFacade:
    ...
    def __init__(self, aio: AsyncTaskport) -> None:
        self._aio = aio
...
class AsyncInlineFacade(_AsyncFacade):
    ...
    directly — you do not need Taskport to run something in the process you are
