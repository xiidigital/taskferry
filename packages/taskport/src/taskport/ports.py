...
```mermaid
flowchart BT
    PRO["taskport_procrastinate"]
    CT["taskport_cloudtasks"]
    CR["taskport_cloudrun"]
    DJ["taskport_django"]
    ...
    CORE["taskport<br/>specs · execution · runtime"]
...
from .errors import ExecutionNotFound, TaskportTimeoutError, UnsupportedCapability
...
class ExecutionBackend(Protocol):
    """What every Taskport backend, of every kind, provides."""
    ...
    def get(self, execution_id: ExecutionId | str) -> Execution:
        ...
        Accepts a Taskport id or, where the adapter documents it, the engine's
        own id — so an operator holding a job id from a dashboard can look one up
        from a process that never submitted it.

        Requires :attr:`~taskport.capabilities.Capability.STATE`.
        """
        ...
...
class InlineBackend(ExecutionBackend, Protocol):
    """Runs an :class:`~taskport.specs.InlineSpec` in the current process."""
...
class TaskBackend(ExecutionBackend, Protocol):
    """Hands a :class:`~taskport.specs.TaskSpec` to a task engine."""
...
class JobBackend(ExecutionBackend, Protocol):
    """Hands a :class:`~taskport.specs.JobSpec` to a batch runtime."""
...
class BaseBackend(ABC):
    ...
    * every capability the spec requires is advertised — otherwise
      :class:`~taskport.errors.UnsupportedCapability` before anything is sent;
...
def accepts(backend: ExecutionBackend, spec: AnySpec) -> bool:
    ...
    fail, and by ``taskport capabilities`` to explain why a route was chosen.
    """
