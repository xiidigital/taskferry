"""``taskport`` — the command line, with no framework attached.

    taskport backends              # what is configured, and where each route goes
    taskport capabilities NAME     # what one backend can actually do
    taskport route --kind task --queue metadata
    taskport submit-job NAME --image ... -- python etl.py
    taskport status EXECUTION_ID --backend NAME
    taskport cancel EXECUTION_ID --backend NAME
    taskport result EXECUTION_ID --backend NAME
    taskport doctor                # is this deployment actually wired up?

Configuration comes from ``TASKPORT_*`` environment variables by default (see
:meth:`~taskport.config.TaskportConfig.from_env`) or from a JSON file passed with
``--config``. Django management commands may wrap this, but Taskport's CLI does
...
from .errors import TaskportError
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="taskport",
        description="Inspect and drive a Taskport deployment.",
    )
    parser.add_argument("--version", action="version", version=f"taskport {__version__}")
    parser.add_argument(
        "--config",
        type=Path,
        metavar="PATH",
        help="JSON file with the Taskport configuration (defaults to TASKPORT_* env vars)",
    )
...
# -- configuration --------------------------------------------------------- #
def _load_config(args: argparse.Namespace) -> TaskportConfig:
    if args.config is None:
        return TaskportConfig.from_env()
    try:
        payload = json.loads(args.config.read_text(encoding="utf-8"))
    except OSError as exc:
        raise TaskportError(f"cannot read {args.config}: {exc}") from exc
    except ValueError as exc:
        raise TaskportError(f"{args.config} is not valid JSON: {exc}") from exc
    return TaskportConfig.from_mapping(payload)


def _runtime(args: argparse.Namespace) -> Taskport:
    return Taskport(config=_load_config(args))
...
def _split_pair(raw: str, flag: str) -> tuple[str, str]:
    ...
    if not sep:
        raise TaskportError(f"{flag} expects NAME=VALUE, got {raw!r}")
    return key, value
