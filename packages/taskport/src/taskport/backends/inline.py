...
The simplest backend and the one that makes Taskport usable before any
...
applies the :class:`~taskport.retry.RetryPolicy` itself, backoff included.
...
class InlineExecutionBackend(BaseBackend):
    """Runs :class:`~taskport.specs.InlineSpec` callables in the calling thread.
    ...
    ages out, ``get()`` raises :class:`~taskport.errors.ExecutionNotFound` — the
