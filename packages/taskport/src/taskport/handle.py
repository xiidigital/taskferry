...
:class:`~taskport.execution.Execution` is a *snapshot*: immutable, cheap, exactly
...
:class:`~taskport.errors.UnsupportedCapability` — a handle never fakes a cancel
...
from .errors import ExecutionError, TaskportTimeoutError, UnsupportedCapability
...
class ExecutionHandle:
    ...
    def wait(self, timeout: float | None = None) -> Execution:
        ...
        Raises:
            TaskportTimeoutError: if ``timeout`` elapses first.
    ...
    def result(self, timeout: float | None = None) -> ExecutionResult:
        ...
        Raises:
            ...
            TaskportTimeoutError: if it did not finish in time.
        ...
        if self._backend.capabilities.supports(Capability.STATE):
            ...
            with contextlib.suppress(TaskportTimeoutError):
                self.refresh()
    ...
    def cancel(self) -> Execution:
        ...
        Raises:
            UnsupportedCapability: if the backend cannot cancel. Taskport does
