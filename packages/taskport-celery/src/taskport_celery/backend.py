...
Translation, in both directions, and nothing else. A :class:`~taskport.specs.TaskSpec`
becomes a Celery task sent to the ``taskport:dispatch`` task; Celery's task state
is translated back to a portable :class:`~taskport.execution.ExecutionState`.
...
flowchart LR
    ...
    subgraph Celery
        SEND["app.send_task('taskport:dispatch', ...)"]
...
result backend is configured, and Taskport will not claim a capability that
...
genuinely queued *and* for a task id it has never heard of. Taskport maps
...
"""Name of the single Celery task that runs every Taskport message."""
...
def map_state(state: object) -> ExecutionState:
    ...
    Celery state must not make Taskport report something false.
    """
    if state is None:
...
class CeleryTaskBackend(BaseBackend):
    """Sends Taskport tasks to a Celery app.
    ...
    Args:
        ...
        dispatch_task: Name of the registered dispatcher task. Must match the name
            passed to :func:`~taskport_celery.register_dispatcher`.
