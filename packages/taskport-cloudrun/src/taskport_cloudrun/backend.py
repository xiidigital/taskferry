...
``Resources(gpu=1)`` raises :class:`~taskport.errors.UnsupportedCapability`
...
from taskport.tracking import ExternalIdIndex, has_prefix
...
class CloudRunJobBackend(BaseBackend):
    ...
    def __init__(
        ...
        *,
        ...
        # with which Taskport id; a full resource path is accepted directly.
...
def _run_v2() -> Any:
    ...
    try:
        ...
        raise ConfigurationError(
            "the Cloud Run backend needs the Google SDK: pip install 'taskport-cloudrun[gcp]'"
        ) from exc
    return run_v2
