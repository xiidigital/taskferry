...
As everywhere else, Taskferry supplies no worker. Service Bus already handles lock
...
logger = logging.getLogger("taskferry.servicebus")
...
def receive_forever(
    ...
    *,
        ...
        while not stop():
            for message in receiver.receive_messages(max_wait_time=max_wait_time):
                try:
                    ...
                    logger.exception(
                        "taskferry: Service Bus message %s failed; abandoning for redelivery",
    ...
    logger.info("taskferry: Service Bus consumer stopping after %d message(s)", processed)
    return processed
