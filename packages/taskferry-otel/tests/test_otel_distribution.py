"""taskferry-otel: the bridge, exercised through the distribution's own surface.

Mirrors the core bridge test but imports from ``taskferry_otel``, proving the
...
from taskferry.core import NoopTracer, get_tracer, set_tracer, span
from taskferry.core.observability import ATTR_PROVIDER, SPAN_JOB_SUBMIT
from taskferry_otel import OTelTracer, configure_opentelemetry
...
def test_the_distribution_reexports_the_bridge() -> None:
    assert configure_opentelemetry.__module__ == "taskferry.core.otel"
    assert OTelTracer.__module__ == "taskferry.core.otel"
...
def bridged(exporter: InMemorySpanExporter) -> Iterator[InMemorySpanExporter]:
    exporter.clear()
    original = get_tracer()
    configure_opentelemetry("taskferry-otel-test")
    try:
        yield exporter
    finally:
        set_tracer(original)
...
def test_spans_reach_opentelemetry(bridged: InMemorySpanExporter) -> None:
    with span(SPAN_JOB_SUBMIT, {ATTR_PROVIDER: "celery"}) as active:
        active.set_attribute("taskferry.job_id", "job_1")
