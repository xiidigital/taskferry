...
:class:`~taskport.specs.Resources` was defined that way, so the least lossy
...
from taskport.core.ids import new_id
...
class KubernetesJobBackend(BaseBackend):
    ...
    def build_manifest(self, spec: JobSpec, job_name: str) -> dict[str, Any]:
        ...
        job_spec: dict[str, Any] = {
            "template": {
                "metadata": {
                    "labels": {**dict(spec.labels), "taskport.dev/job": _dns_name(spec.job)}
                },
                "spec": pod_spec,
            },
        ...
        return {
            ...
            "metadata": {
                "name": job_name,
                "labels": {**dict(spec.labels), "app.kubernetes.io/managed-by": "taskport"},
...
def _dns_name(value: str) -> str:
    ...
    return cleaned.strip("-") or "taskport-job"
