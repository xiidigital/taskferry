"""Tests for taskferry-core. Targets the >=90% coverage bar for core (section 38)."""
...
from taskferry.core.observability import ATTR_PROVIDER, SPAN_JOB_SUBMIT
...
def test_is_taskferry_id_false_for_plain_string() -> None:
    assert not is_taskferry_id("plainstring")
...
def test_capability_set_contains_require_and_missing() -> None:
    ...
    assert isinstance(exc.value, TaskferryError)
...
def test_resolve_factory_forms_and_errors() -> None:
    assert resolve_factory("taskferry.core:JsonSerializer") is JsonSerializer
    assert resolve_factory("taskferry.core.JsonSerializer") is JsonSerializer
    with pytest.raises(ConfigurationError):
        resolve_factory("nomodulehere:Thing")
    with pytest.raises(ConfigurationError):
        resolve_factory("taskferry.core:__version__")  # not callable
    with pytest.raises(ConfigurationError):
...
def test_lazy_registry_spec_and_configure() -> None:
    ...
    reg.register_spec("json", "taskferry.core:JsonSerializer", sort_keys=False)
    assert isinstance(reg["json"], JsonSerializer)

    reg.configure({"other": {"factory": "taskferry.core:JsonSerializer"}})
...
def test_set_tracer_is_used_then_restored() -> None:
    ...
    try:
        set_tracer(_Tracer())
        with span("taskferry.task.enqueue") as s:
            s.set_attribute("k", "v")
