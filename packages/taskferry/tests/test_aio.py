...
from taskferry.errors import ExecutionError, ExecutionNotFound, RoutingError, UnsupportedCapability


@pytest.fixture
async def aio() -> AsyncTaskferry:
...
class TestTheSameVocabulary:
    ...
    def test_method_names_match_the_sync_runtime(self) -> None:
        ...
        assert sync <= set(dir(AsyncTaskferry))

    def test_handle_method_names_match(self) -> None:
        from taskferry import ExecutionHandle
...
class TestThreePrimitives:
    async def test_inline(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_inline_run_returns_the_value(self, aio: AsyncTaskferry) -> None:
        assert await aio.inline.run(tasks_fixture.add, 20, 22) == 42

    async def test_task(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_job(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_an_async_task_function_is_awaited(self, aio: AsyncTaskferry) -> None:
...
class TestItReallyDoesNotBlockTheLoop:
    ...
    async def test_the_loop_keeps_running_during_a_slow_job(self, aio: AsyncTaskferry) -> None:
        ticks = 0
    ...
    async def test_submissions_run_concurrently(self, aio: AsyncTaskferry) -> None:
...
class TestSharedRuntime:
    ...
    def test_wrapping_shares_everything(self) -> None:
        sync = Taskferry.local()
        aio = AsyncTaskferry(sync)
...
class TestRoutingAndCapabilitiesAreShared:
    ...
    async def test_routing_is_the_same(self) -> None:
        ...
        sync = Taskferry.from_mapping(config)
        aio = AsyncTaskferry(sync)
    ...
    async def test_capability_refusals_are_identical(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_a_wrong_kind_backend_is_refused(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_an_unknown_id_explains_itself(self, aio: AsyncTaskferry) -> None:
        with pytest.raises(ExecutionNotFound, match="pass backend="):
            await aio.get("task_never_submitted")

    async def test_cancel_is_refused_when_unsupported(self, aio: AsyncTaskferry) -> None:
        handle = await aio.inline.submit(tasks_fixture.add, 1, 1)
        with pytest.raises(UnsupportedCapability, match="cancel"):
            await handle.cancel()

    async def test_cancel_works_where_supported(self, aio: AsyncTaskferry) -> None:
        handle = await aio.jobs.submit(
...
class TestFailures:
    async def test_a_failure_raises_from_result_not_from_submit(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_a_timeout_raises_rather_than_hanging(self, aio: AsyncTaskferry) -> None:
    ...
    async def test_a_terminal_handle_is_not_re_polled(self, aio: AsyncTaskferry) -> None:
        handle = await aio.inline.submit(tasks_fixture.add, 1, 1)
...
class TestLifecycle:
    async def test_async_context_manager(self) -> None:
        async with AsyncTaskferry.local() as runtime:
    ...
    def test_constructors_mirror_the_sync_ones(self) -> None:
        for build in (
            lambda: AsyncTaskferry.local(),
            lambda: AsyncTaskferry.from_env({}),
            lambda: AsyncTaskferry.from_mapping(
                {"backends": {"i": {"factory": "inline"}}, "defaults": {"inline": "i"}}
            ),
        ):
            runtime = build()
            assert isinstance(runtime, AsyncTaskferry)
            runtime.sync.close()
...
class TestBackendAsyncPort:
    ...
    async def test_the_default_async_path_delegates_correctly(self) -> None:
        from taskferry.backends.thread import ThreadTaskBackend
    ...
    async def test_an_adapter_can_override_the_async_path(self) -> None:
        ...
        await backend.asubmit(
            tasks_fixture
            and __import__("taskferry").specs.InlineSpec(func=tasks_fixture.add, args=(1, 1))
        )
        assert calls == ["native"]
