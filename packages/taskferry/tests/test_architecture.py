...
still right — because "taskferry imports no framework and no provider SDK" is not
...
2. **Runtime check** — imports ``taskferry`` in a clean subprocess and inspects
...
3. **Metadata check** — reads the declared dependencies. Catches the case where
   the code is clean but ``pip install taskferry`` still drags Django in.
"""
...
TASKFERRY_SRC = PACKAGES_DIR / "taskferry" / "src" / "taskferry"

#: Nothing in this list may appear anywhere in the taskferry distribution.
...
#: The only third-party imports allowed inside taskferry, each with the reason it
#: does not violate "no dependencies". Both are proven harmless by
#: ``test_importing_taskferry_pulls_in_nothing_heavy``, which checks what actually
...
ALLOWED_THIRD_PARTY: dict[str, set[str]] = {
    # taskferry.contract ships the reusable suites an independently released
...
class TestCoreImportsNothing:
    """`taskferry` depends on the standard library and nothing else."""

    @pytest.mark.parametrize("path", _python_files(TASKFERRY_SRC), ids=lambda p: p.name)
    def test_no_forbidden_import_in_source(self, path: Path) -> None:
        ...
        assert not offending, (
            ...
            f"taskferry must not depend on a framework or a provider SDK — that belongs "
    ...
    def test_no_undeclared_third_party_import(self) -> None:
        ...
        for path in _python_files(TASKFERRY_SRC):
            relative = str(path.relative_to(TASKFERRY_SRC))
            allowed = ALLOWED_THIRD_PARTY.get(relative, set())
            outside = {
                root
                for root in _imported_roots(path)
                if root not in stdlib and root != "taskferry" and not root.startswith("_")
            }
            if outside - allowed:
                unexpected[relative] = sorted(outside - allowed)
        assert not unexpected, (
            f"undeclared third-party imports in taskferry: {unexpected}. The distribution "
    ...
    def test_importing_taskferry_pulls_in_nothing_heavy(self) -> None:
        ...
        assert not leaked, (
            f"importing taskferry transitively imported {leaked}. Every provider SDK must be "
            f"imported lazily, inside the method that needs a client."
        )

    def test_taskferry_declares_no_dependencies(self) -> None:
        """Clean code is not enough: the metadata has to be clean too."""
        pyproject = tomllib.loads(
            (PACKAGES_DIR / "taskferry" / "pyproject.toml").read_text(encoding="utf-8")
        )
        dependencies = pyproject["project"].get("dependencies", [])
        assert dependencies == [], (
            f"taskferry declares {dependencies}. `pip install taskferry` must install nothing "
            f"but taskferry."
        )
    ...
    def test_public_api_needs_no_deep_imports(self) -> None:
        ...
        missing = expected - set(taskferry.__all__)
        assert not missing, f"missing from the public API: {sorted(missing)}"
        for name in taskferry.__all__:
            assert hasattr(taskferry, name), f"__all__ lists {name!r} but it is not importable"


class TestAdapterDependencyGraph:
    """Adapters depend on taskferry. Nothing depends on an adapter."""
    ...
    def test_adapter_depends_on_taskferry_only(self, dist: str) -> None:
        """Required dependencies are taskferry plus, at most, one framework."""
        pyproject = tomllib.loads(
            (PACKAGES_DIR / dist / "pyproject.toml").read_text(encoding="utf-8")
        )
        required = pyproject["project"].get("dependencies", [])
        assert any(dep.startswith("taskferry") for dep in required), (
            f"{dist} must depend on taskferry"
        )
        ...
        provider_deps = [
            dep
            for dep in required
            if not dep.startswith("taskferry") and not dep.startswith("django")
        ]
    ...
    def test_adapter_owns_a_top_level_module(self, dist: str) -> None:
        """No adapter contributes to the ``taskferry`` package.

        ``taskferry`` ships ``__init__.py``, so it is a regular package. Another
        distribution dropping files into it would break under editable installs
        and is forbidden by PEP 420 anyway. See ADR-0013.
        """
        src = PACKAGES_DIR / dist / "src"
        assert not (src / "taskferry").exists(), (
            f"{dist} still contributes to the taskferry namespace; it must own "
...
class TestNoQueueReimplementation:
    """Taskferry routes work to engines. It does not become one.
    ...
    def test_core_does_not_grow_engine_features(self) -> None:
        ...
        assert not found, "taskferry is reimplementing engine features:\n" + "\n".join(found)
