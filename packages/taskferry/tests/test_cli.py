...
from taskferry.cli import EXIT_ERROR, EXIT_OK, build_parser, main


@pytest.fixture
def config_file(tmp_path: Path) -> Path:
    """A JSON configuration file, the form the CLI reads outside a framework."""
    path = tmp_path / "taskferry.json"
    path.write_text(
        json.dumps(
            {
                "backends": {
...
class TestConfigLoading:
    ...
    def test_without_a_config_it_falls_back_to_the_environment(
        ...
        """A container with no TASKFERRY_* set still starts and still works."""
        for key in list(dict(__import__("os").environ)):
            if key.startswith("TASKFERRY_"):
