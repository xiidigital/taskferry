...
from taskferry.config import BackendConfig
...
from taskferry.specs import InlineSpec
...
class TestAcceptanceCriteria:
    ...
    def test_pure_python_with_no_infrastructure(self) -> None:
        from taskferry import Taskferry as ImportedTaskferry

        rt = ImportedTaskferry.local()
    ...
    def test_all_three_primitives_from_one_runtime(self, runtime: Taskferry) -> None:
    ...
    def test_the_three_kinds_stay_distinct(self, runtime: Taskferry) -> None:
...
class TestInlineFacade:
    def test_run_returns_the_value_directly(self, runtime: Taskferry) -> None:
        assert runtime.inline.run(tasks_fixture.add, 20, 22) == 42

    def test_a_closure_works(self, runtime: Taskferry) -> None:
        """Inline is the primitive where no importable name is needed."""
        offset = 10
        assert runtime.inline.run(lambda x: x + offset, 5) == 15

    def test_a_failure_surfaces_from_result_not_from_submit(self, runtime: Taskferry) -> None:
    ...
    async def test_an_async_callable_is_awaited(self, runtime: Taskferry) -> None:
        handle = runtime.inline.submit(tasks_fixture.async_double, 21)
        assert handle.result().value == 42

    def test_retries_are_applied_by_the_inline_backend(self, runtime: Taskferry) -> None:
    ...
    def test_a_timeout_is_refused_rather_than_ignored(self, runtime: Taskferry) -> None:
...
class TestTaskFacade:
    def test_a_callable_is_accepted_and_registered(self, runtime: Taskferry) -> None:
    ...
    def test_a_name_string_is_accepted(self, runtime: Taskferry) -> None:
        handle = runtime.tasks.submit("tasks_fixture:add", 5, 6)
        assert handle.result(10).value == 11

    def test_a_lambda_is_rejected_with_an_explanation(self, runtime: Taskferry) -> None:
    ...
    def test_a_failure_is_recorded_on_the_execution(self, runtime: Taskferry) -> None:
    ...
    def test_delay_is_honoured(self, runtime: Taskferry) -> None:
    ...
    def test_priority_is_refused_by_a_fifo_backend(self, runtime: Taskferry) -> None:
    ...
    def test_an_idempotency_key_is_refused_when_unsupported(self, runtime: Taskferry) -> None:
        with pytest.raises(UnsupportedCapability, match="deduplication"):
            runtime.tasks.submit(tasks_fixture.add, 1, 1, idempotency_key="k")

    async def test_an_async_task_function_runs(self, runtime: Taskferry) -> None:
        handle = runtime.tasks.submit(tasks_fixture.async_double, 21)
        assert handle.result(10).value == 42


class TestJobFacade:
    def test_a_failing_job_reports_its_exit_code(self, runtime: Taskferry) -> None:
    ...
    def test_environment_variables_reach_the_child(self, runtime: Taskferry) -> None:
    ...
    def test_a_gpu_request_is_refused_by_a_subprocess_backend(self, runtime: Taskferry) -> None:
        """The assertion that matters most: never silently run GPU work on a CPU."""
        from taskferry import Resources

        with pytest.raises(UnsupportedCapability, match="gpu"):
            runtime.jobs.submit("train", command=["true"], resources=Resources(gpu=1))

    def test_a_timeout_really_kills_the_child(self, runtime: Taskferry) -> None:
        handle = runtime.jobs.submit(
    ...
    def test_cancel_stops_a_running_job(self, runtime: Taskferry) -> None:
        handle = runtime.jobs.submit(
...
class TestConfiguration:
    ...
    def test_from_mapping_builds_the_whole_configuration(self) -> None:
        config = TaskferryConfig.from_mapping(
            {
    ...
    def test_from_env_reads_json_structure(self) -> None:
        config = TaskferryConfig.from_env(
            {
                "TASKFERRY_BACKENDS": '{"t": {"factory": "thread"}}',
                "TASKFERRY_ROUTES": '[{"kind": "task", "queue": "q", "backend": "t"}]',
                "TASKFERRY_DEFAULT_TASK": "t",
            }
        )
    ...
    def test_from_env_falls_back_to_local_when_nothing_is_set(self) -> None:
        """A container that forgot to configure Taskferry still starts and still runs."""
        config = TaskferryConfig.from_env({})
        assert set(config.backends) == {"inline", "thread", "subprocess"}

    def test_malformed_env_json_says_which_variable(self) -> None:
        with pytest.raises(ConfigurationError, match="TASKFERRY_BACKENDS"):
            TaskferryConfig.from_env({"TASKFERRY_BACKENDS": "{not json"})

    def test_configuration_is_immutable(self) -> None:
        config = TaskferryConfig.local()
...
class TestRuntimeWiring:
    def test_backends_are_built_lazily_and_cached(self) -> None:
        ...
        from taskferry import register_backend, unregister_backend
    ...
    def test_a_backend_of_the_wrong_kind_is_refused(self, runtime: Taskferry) -> None:
    ...
    def test_injected_backends_take_precedence(self) -> None:
        ...
        rt = Taskferry(config=TaskferryConfig.local(), backends={"inline": injected})
        assert rt.backend("inline") is injected
        rt.close()

    def test_get_finds_the_owning_backend_without_being_told(self, runtime: Taskferry) -> None:
        handle = runtime.tasks.submit(tasks_fixture.add, 1, 1)
        handle.wait(10)
        assert runtime.get(handle.id).state is ExecutionState.SUCCEEDED

    def test_get_on_an_unknown_id_explains_how_to_recover(self, runtime: Taskferry) -> None:
    ...
    def test_the_execution_index_is_bounded(self) -> None:
        ...
        rt = Taskferry(config=TaskferryConfig.local().evolve(max_tracked_executions=3))
    ...
    def test_correlation_is_attached_automatically(self, runtime: Taskferry) -> None:
...
class TestExecutionHandle:
    def test_a_terminal_snapshot_is_not_re_polled(self, runtime: Taskferry) -> None:
        """Polling a finished execution wastes an engine round-trip."""
        handle = runtime.inline.submit(tasks_fixture.add, 1, 1)
        first = handle.refresh()
        assert handle.refresh() is first

    def test_cancel_is_refused_when_unsupported(self, runtime: Taskferry) -> None:
        handle = runtime.inline.submit(tasks_fixture.add, 1, 1)
        with pytest.raises(UnsupportedCapability, match="cancel"):
            handle.cancel()

    def test_capabilities_are_reachable_from_the_handle(self, runtime: Taskferry) -> None:
        """Ask before you act — that is the whole capability contract."""
        handle = runtime.tasks.submit(tasks_fixture.add, 1, 1)
        assert Capability.CANCEL in handle.capabilities

    def test_wait_times_out_rather_than_hanging(self, runtime: Taskferry) -> None:
        from taskferry.errors import TaskferryTimeoutError

        handle = runtime.jobs.submit(
            "slow", command=[sys.executable, "-c", "import time; time.sleep(10)"]
        )
        with pytest.raises(TaskferryTimeoutError):
            handle.wait(0.2)
        handle.cancel()
...
class TestHooks:
    def test_hooks_observe_the_inline_lifecycle(self) -> None:
        ...
        rt = Taskferry.local(hooks=[Recorder()])
    ...
    def test_submit_side_and_execute_side_hooks_are_not_ordered_against_each_other(
        ...
        The guarantee Taskferry makes is per-side: ``before_submit`` precedes
    ...
    def test_a_failing_hook_never_breaks_the_work(self) -> None:
        ...
        rt = Taskferry.local(hooks=[Broken()])
    ...
    def test_a_retry_is_reported(self) -> None:
        ...
        rt = Taskferry.local(hooks=[Watcher()])
        rt.inline.submit(
            tasks_fixture.flaky,
            "hook-retry",
            2,
...
class TestThreadSafety:
    def test_concurrent_submits_from_many_threads(self, runtime: Taskferry) -> None:
...
class TestBackendOptionsPortability:
    def test_options_for_another_engine_are_ignored(self, runtime: Taskferry) -> None:
        """One spec must travel unchanged from one engine to another."""
        from taskferry import BackendOptions
...
class TestAsyncModel:
    ...
    async def test_a_sync_callable_also_works_from_async_code(self, runtime: Taskferry) -> None:
        assert runtime.inline.run(tasks_fixture.add, 20, 22) == 42

    async def test_submitting_a_task_from_async_code_works(self, runtime: Taskferry) -> None:
        """submit() is a short local operation, so blocking the loop briefly is fine."""
        handle = runtime.tasks.submit(tasks_fixture.async_double, 21)
        assert handle.result(10).value == 42

    def test_async_capability_is_advertised_only_where_true(self, runtime: Taskferry) -> None:
        from taskferry import Capability
    ...
    def test_async_callable_detection_sees_through_partials(self) -> None:
        ...
        from taskferry.functions import is_async_callable
...
class TestImmutabilityGuarantees:
    ...
    def test_every_model_type_is_frozen(self) -> None:
        ...
        from taskferry import (
            TaskferryConfig as _TaskferryConfig,
        )
        from taskferry import (
            TimeoutPolicy as _TimeoutPolicy,
        )
        from taskferry.specs import ExecutionSpec
    ...
    def test_capability_sets_are_hashable_so_they_can_be_cached(self) -> None:
        from taskferry import CapabilitySet
