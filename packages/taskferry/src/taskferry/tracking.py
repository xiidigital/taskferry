"""Pairing Taskferry ids with engine ids — the bookkeeping every adapter needs.

A Taskferry id is Taskferry's. Procrastinate mints an integer, Cloud Run mints a
...
limitation of looking an execution up by Taskferry id, not something to paper
...
class ExternalIdIndex:
    """A bounded, thread-safe, bidirectional map of Taskferry id ↔ engine id.
