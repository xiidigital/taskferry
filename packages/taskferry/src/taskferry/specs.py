...
into :class:`~taskferry.core.config.ProviderOptions`, namespaced by backend:
...
from taskferry.core.config import ProviderOptions
from taskferry.core.correlation import Correlation
from taskferry.core.serialization import ensure_json_serializable
from taskferry.core.typing import JSONObject, JSONValue
...
Aliased from :class:`taskferry.core.config.ProviderOptions` — same object, a name
...
class ExecutionSpec:
    ...
    Attributes:
        ...
        profile: Logical resource profile (``"heavy"``, ``"gpu"``). The primary
            ...
            job to decide.
        retry: Portable retry intent. See :mod:`taskferry.retry`.
        ...
        idempotency_key: Caller-supplied deduplication key. Backends advertising
            ...
            key. Taskferry makes **no** exactly-once promise; this is a tool for
...
"""Union of every spec kind — what :meth:`taskferry.Taskferry.submit` accepts."""
