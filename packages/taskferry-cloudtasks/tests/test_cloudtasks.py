...
from taskferry.errors import ConfigurationError, SerializationError, SubmissionError
from taskferry.functions import FunctionRegistry
from taskferry.ports import ExecutionBackend
from taskferry_cloudtasks import CloudTasksBackend, handle_request, parse_request
from taskferry_cloudtasks.backend import MESSAGE_VERSION, build_message
...
def make_backend(**overrides: Any) -> CloudTasksBackend:
    return CloudTasksBackend(
        ...
        url=overrides.pop("url", "https://svc.run.app/_taskferry/execute"),
...
class TestSubmission:
    ...
    def test_an_oidc_token_is_attached_when_a_service_account_is_configured(self) -> None:
        ...
        assert oidc["audience"] == "https://svc.run.app/_taskferry/execute"
    ...
    def test_correlation_travels_as_http_headers(self) -> None:
        ...
        assert headers["taskferry-correlation-id"] == correlation.correlation_id
...
class TestReceiver:
    ...
    def test_a_version_mismatch_is_rejected(self) -> None:
        with pytest.raises(SerializationError, match="unsupported taskferry envelope version"):
            parse_request(json.dumps({"taskferry": "99", "task": "m:f"}).encode())
    ...
    def test_a_body_without_a_task_is_rejected(self) -> None:
        with pytest.raises(SerializationError, match="no 'task'"):
            parse_request(json.dumps({"taskferry": MESSAGE_VERSION}).encode())
    ...
    def test_headers_restore_the_correlation(self) -> None:
        ...
        correlation = Correlation.start()
        body = json.dumps({"taskferry": MESSAGE_VERSION, "task": "tests:observe"}).encode()
