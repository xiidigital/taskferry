...
2. nothing about Django reaches ``taskport`` — the same configuration works in a
...
import taskport_django
from taskport import Capability, ExecutionKind, ExecutionState, Taskport, TaskportConfig
from taskport_django import TaskportBackend, config_from_settings, get_runtime, reset_runtime
from taskport_django.checks import ID_LOCAL_IN_PRODUCTION, ID_NOT_WIRED, check_taskport
...
class TestConfigFromSettings:
    ...
    @override_settings(TASKPORT={"backends": {}, "defaults": {"task": "missing"}})
...
class TestTaskportBackend:
    ...
    def test_every_spec_points_at_the_one_dispatcher(self) -> None:
        ...
        See taskport_django.execute for the reasoning. The consequence tested
        here is that only one module needs to be importable by a worker, which
        makes an import allowlist trivial to write.
        """
        from taskport_django.execute import DISPATCH_REF

        spec = TaskportBackend("default", {}).build_spec(add, [1, 2], {})
    ...
    def test_arguments_travel_as_data_inside_the_payload(self) -> None:
        spec = TaskportBackend("default", {}).build_spec(add, [1, 2], {"extra": "x"})
    ...
    def test_the_dispatcher_really_runs_the_django_task(self) -> None:
        ...
        from taskport_django.execute import run_task
    ...
    def test_a_pinned_backend_bypasses_routing(self) -> None:
        backend = TaskportBackend("pinned", {"OPTIONS": {"backend": "thread"}})
    ...
    def test_capabilities_come_from_the_routed_engine_not_a_hardcoded_flag(self) -> None:
        ...
        backend = TaskportBackend("default", {})
        capabilities = backend.taskport_capabilities()
    ...
    def test_get_result_refuses_rather_than_fabricating(self) -> None:
        ...
        backend = TaskportBackend("default", {})
        with pytest.raises(TaskportError, match=r"runtime\.get"):
    ...
    def test_the_state_mapping_covers_every_portable_state(self) -> None:
        ...
        from taskport_django.backend import _STATUS_MAP
...
class TestOnCommit:
    ...
    def test_submission_waits_for_the_commit(self) -> None:
        ...
        from taskport_django import submit_on_commit
...
class TestSystemChecks:
    def test_a_valid_configuration_produces_no_errors(self) -> None:
        errors = [message for message in check_taskport() if message.is_serious()]
        assert not errors, [message.msg for message in errors]

    @override_settings(
        TASKPORT={"backends": {"broken": {"factory": "cloudrun"}}, "defaults": {"job": "broken"}}
    )
    ...
    def test_a_configured_but_unwired_taskport_is_flagged(self) -> None:
        messages = check_taskport()
    ...
    @override_settings(TASKPORT={"backends": {"t": {"factory": "thread"}}})
    def test_missing_defaults_are_flagged(self) -> None:
        from taskport_django.checks import ID_NO_DEFAULT
...
class TestNoLeakage:
    ...
    def test_taskport_works_in_a_process_without_django(self) -> None:
        """Run in a subprocess that never imports Django, and prove work happens."""
        probe = (
            "import sys;"
            "import taskport;"
            "assert 'django' not in sys.modules, 'taskport imported django';"
            "rt = taskport.Taskport.local();"
    ...
    def test_the_same_config_dict_serves_django_and_plain_python(self) -> None:
        ...
        raw: dict[str, Any] = settings.TASKPORT
        standalone = Taskport.from_mapping(raw)
        try:
    ...
    def test_the_django_package_exposes_only_the_bridge(self) -> None:
        ...
        assert set(taskport_django.__all__) == {
            "TaskportBackend",
            "__version__",
            "config_from_settings",
            "get_runtime",
