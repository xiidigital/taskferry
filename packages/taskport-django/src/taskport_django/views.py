...
The agnostic receiver (:func:`taskport.envelope.execute_envelope`) takes bytes, so
...
**This endpoint runs code and Taskport does not authenticate it.** Verify the
...
from taskport.errors import FunctionResolutionError, SerializationError
from taskport.functions import FunctionRegistry

logger = logging.getLogger("taskport.django")
    ...
    *,
    ...
    Args:
        ...
        authenticate: Called with the request; return ``False`` to reject with
            403. Taskport cannot do this for you — it never sees your framework's
...
def _execute(request: HttpRequest, registry: FunctionRegistry | None) -> HttpResponse:
    ...
    try:
        ...
        logger.warning("taskport: rejected a malformed envelope: %s", exc)
        return JsonResponse({"error": str(exc)}, status=400)
    except FunctionResolutionError as exc:
        # The name does not resolve, or the allowlist refused it. Permanent.
        logger.warning("taskport: could not resolve a pushed task: %s", exc)
