...
from taskport import BackendOptions, Capability, ExecutionState, RetryPolicy, TaskSpec
...
from taskport_dramatiq import DramatiqTaskBackend, dispatch
...
class TestSending:
    def test_the_envelope_is_the_actor_argument(self) -> None:
        ...
        assert envelope["taskport"] == ENVELOPE_VERSION
...
class TestActorResolution:
    ...
    def test_a_dotted_path_is_resolved_lazily(self) -> None:
        ...
        target = DramatiqTaskBackend(actor="taskport_dramatiq:__version__")
    ...
    def test_an_actor_can_come_from_a_broker(self) -> None:
        actor = FakeActor()
        broker = SimpleNamespace(actors={"taskport_execute": actor})
...
class TestWorker:
    ...
    def test_a_version_mismatch_is_rejected(self) -> None:
        ...
        with pytest.raises(SerializationError, match="unsupported taskport envelope version"):
            dispatch({"taskport": "99", "task": "tests:add"})
