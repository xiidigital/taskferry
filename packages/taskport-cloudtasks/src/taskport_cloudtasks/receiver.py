...
them, and is why ``taskport-cloudtasks`` depends on neither Django nor FastAPI.
...
behind IAM. Taskport cannot do this for you: it never sees your framework's
...
def parse_request(body: bytes | str) -> CloudTasksMessage:
    ...
    Raises:
        SerializationError: when the body is not a Taskport message of a version
...
def _correlation_from(message: CloudTasksMessage, headers: Mapping[str, str] | None) -> Correlation:
    ...
    if headers:
        ...
        if "taskport-correlation-id" in normalised:
