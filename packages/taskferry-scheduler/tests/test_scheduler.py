"""Tests for taskferry_scheduler: triggers, LocalScheduler, Cloud Scheduler, contract."""
...
from taskferry.core import UnsupportedCapabilityError
...
from taskferry_scheduler.adapters.gcp import CloudSchedulerScheduler, map_scheduler_state
from taskferry_scheduler.contract import SchedulerContract
...
def test_gcp_create_builds_cron_http_job() -> None:
    ...
    handle = scheduler.create(
        Schedule(
            ...
            HttpTarget("https://svc/_taskferry/execute", headers={"X": "1"}),
        )
    )
    ...
    assert job["http_target"]["uri"].endswith("/_taskferry/execute")
