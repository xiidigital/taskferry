"""``taskferry_scheduler`` — portable "when to fire" triggering.
...
45). Plain Python, no Django required (section 57):
    ...
    handle = schedulers["default"].create(
        Schedule(
            ...
            target=HttpTarget("https://svc/_taskferry/execute"),
        )
    )
...
``taskferry_scheduler.adapters`` and import their SDK lazily (section 31).
"""
