"""Tests for taskferry_events: model, in-memory bus, Pub/Sub adapter, contract."""
...
from taskferry.core import Correlation, SerializationError
from taskferry_events import (
    Event,
    EventCapability,
    InMemoryEventBus,
    publishers,
)
from taskferry_events.adapters.gcp import PubSubPublisher
...
def test_event_to_cloudevent_is_json_safe() -> None:
    ...
    assert dumped["taskferrycorrelation"]["taskferry-correlation-id"] == corr.correlation_id
...
def test_pubsub_publish_builds_message() -> None:
    ...
    assert attributes["taskferry-event-type"] == "dataset.updated"
    assert attributes["taskferry-event-subject"] == "ds-9"
