...
CloudEvents schema — the format Taskferry already speaks.
"""
...
class EventGridPublisher(BaseEventPublisher):
        ...
        if self._client is None:
            ...
            try:
                ...
                raise ProviderError(
                    ...
                    "EventGridPublisher; install taskferry-events[azure]",
                    provider="azure",
                ) from exc
    ...
    def _publish(self, event: Event) -> PublishResult:
        ...
        # Event Grid does not return a per-event id; the Taskferry id stands in.
