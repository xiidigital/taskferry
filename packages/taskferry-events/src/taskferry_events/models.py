...
class Event:
    ...
    type: str
    source: str
    id: TaskferryId = field(default_factory=lambda: new_id("evt"))
    ...
    def to_cloudevent(self) -> JSONObject:
        ...
        if self.correlation is not None:
            envelope["taskferrycorrelation"] = cast(
                "JSONObject", dict(self.correlation.to_headers())
            )
        return envelope
...
def event_attributes(event: Event) -> Mapping[str, str]:
    ...
    attributes = {
        "taskferry-event-type": event.type,
        "taskferry-event-source": event.source,
    }
    if event.subject is not None:
        attributes["taskferry-event-subject"] = event.subject
