"""``TaskferryBackend`` — a Django Tasks backend that delegates to Taskferry.
...
a :class:`~taskferry.specs.TaskSpec` and handing that to the runtime, which routes
...
``TASKFERRY`` from a thread pool to Cloud Tasks and ``supports_get_result``
...
from taskferry import CapabilitySet, ExecutionState, Taskferry, TaskSpec
...
#: coarser than Taskferry's, so several states collapse; the mapping is written
...
class TaskferryBackend(BaseTaskBackend):
    """Routes ``django.tasks`` enqueues through a Taskferry runtime.
    ...
    Taskferry backend. Leave it out and the router decides from the task's queue,
    ...
    def __init__(self, alias: str, params: dict[str, Any]) -> None:
        ...
        self._runtime_override: Taskferry | None = None
        ...
        # the wrong one here: a Taskferry queue is a *routing key*, and which
    ...
    @property
    def runtime(self) -> Taskferry:
        """The Taskferry runtime. Injectable for tests via :meth:`use_runtime`."""
        return self._runtime_override if self._runtime_override is not None else get_runtime()

    def use_runtime(self, runtime: Taskferry | None) -> None:
        """Point this backend at a specific runtime. For tests."""
        self._runtime_override = runtime

    # -- capabilities ---------------------------------------------------------- #
    def taskferry_capabilities(self, queue: str = "default") -> CapabilitySet:
        """What the backend this queue routes to can actually do.

        Returns the :class:`~taskferry.CapabilitySet` itself rather than a plain
    ...
    def build_spec(self, task: Task, args: list[Any], kwargs: dict[str, Any]) -> TaskSpec:
        ...
        Every spec points at :data:`~taskferry_django.execute.DISPATCH_REF`, with
        the Django task path travelling as data. See
        :mod:`taskferry_django.execute` for why the task's own path cannot be used
        ...
        :attr:`~taskferry.specs.ExecutionSpec.name` carries the real task name, so
    ...
    def _task_result(
        ...
        return TaskResult(
            task=task,
            # Django caps result ids at 64 characters; a Taskferry id is well
    ...
    def get_result(self, result_id: str) -> TaskResult:
        ...
        Raises:
            TaskferryError: when the routed backend cannot report state. Django's
