...
Taskferry could special-case that, but only by teaching the core what a Django
...
* **Allowlisting is trivial.** Only ``taskferry_django.execute`` needs to be
...
:attr:`~taskferry.specs.ExecutionSpec.name` carries the real task name and routing
...
from taskferry.core.typing import JSONValue
...
DISPATCH_REF = "taskferry_django.execute:run_task"
