...
from taskferry_sqs import SQSTaskBackend, poll_forever, process_event, process_message
        ...
        def event(*tasks: str) -> dict[str, Any]:
            return {
                "Records": [
                    {
                        "messageId": f"m{n}",
                        "body": json.dumps({"taskferry": ENVELOPE_VERSION, "task": task}),
                    }
    ...
    def test_an_all_good_batch_reports_no_failures(self) -> None:
        ...
        event = {
            "Records": [
                {
                    "messageId": "m0",
                    "body": json.dumps({"taskferry": ENVELOPE_VERSION, "task": "tests:ok"}),
                }
            ]
        }
            ...
            messages=[
                {
                    "MessageId": "m1",
                    "ReceiptHandle": "r1",
                    "Body": json.dumps({"taskferry": ENVELOPE_VERSION, "task": "tests:missing"}),
                }
            ]
        )
        calls = {"n": 0}
