...
Taskferry supplies **no worker daemon**. What it supplies is the function that
...
SQS delivery is at-least-once and Taskferry does not change that. A handler that
...
logger = logging.getLogger("taskferry.sqs")
...
def process_event(
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            logger.exception("taskferry: SQS message %s failed", message_id)
...
def poll_forever(
    ...
    *,
    ...
    while not stop():
        ...
        for message in response.get("Messages", []) or []:
            try:
                ...
                logger.exception(
                    "taskferry: SQS message %s failed; leaving it for redelivery",
    ...
    logger.info("taskferry: SQS consumer stopping after %d message(s)", processed)
    return processed
