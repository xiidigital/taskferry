...
Every Taskport transport carries the same JSON envelope — one function name, JSON
arguments, correlation, retry intent. It lives in :mod:`taskport.envelope` so
...
def check_version(message: object) -> None:
    ...
    Thin wrapper over :func:`taskport.envelope.read_envelope` that raises
