...
* ``RETRY`` — the engine owns retries. The portable policy travels in the
  ...
  delay. Taskport never loops in-process pretending to retry;
...
from taskport.tracking import ExternalIdIndex, is_digits

from .message import build_message

DEFAULT_DISPATCH_TASK = "taskport:dispatch"
"""Name of the single Procrastinate task that runs every Taskport message."""
...
#: newer releases; both map to the Taskport state that describes what an operator
...
def map_status(status: object) -> ExecutionState:
    ...
    status must not make Taskport report something false.
    """
    if status is None:
...
class ProcrastinateTaskBackend(BaseBackend):
    """Defers Taskport tasks as Procrastinate jobs.
    ...
    def __init__(
        ...
        *,
        ...
        self._name = name
        # Procrastinate knows nothing about Taskport ids, so remember the
    ...
    def _get(self, execution_id: ExecutionId) -> Execution:
        ...
        Taskport ids are Taskport's; Procrastinate never sees one. So ``get``
        accepts either — a Taskport id when this process submitted it, or the
