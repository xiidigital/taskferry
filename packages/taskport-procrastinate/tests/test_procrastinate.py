...
adapter's *own* code is exercised, and that ``taskport`` needs neither
...
from taskport_procrastinate import ProcrastinateTaskBackend, map_status
from taskport_procrastinate.message import (
    MESSAGE_VERSION,
    build_message,
    check_version,
    decode_retry,
...
from taskport_procrastinate.worker import execute_message
...
class FakeJob:
    def __init__(self, job_id: int, status: str = "todo", queue: str = "default") -> None:
        ...
        self.task_name = "taskport:dispatch"
        self.attempts = 0
...
class TestSpecTranslation:
    ...
    def test_the_taskport_id_is_not_the_procrastinate_id(
...
class TestConfiguration:
    ...
    def test_a_dotted_app_path_is_resolved_lazily(self) -> None:
        ...
        backend = ProcrastinateTaskBackend(app="taskport_procrastinate:__version__")
...
class TestMessage:
    ...
    def test_no_retry_encodes_to_nothing(self) -> None:
        from taskport.retry import NO_RETRY
...
class TestWorker:
    ...
    def test_a_message_without_a_task_is_rejected(self) -> None:
        with pytest.raises(ValueError, match="no 'task'"):
            execute_message({"taskport": MESSAGE_VERSION})
    ...
    def test_correlation_survives_the_round_trip(self) -> None:
        from taskport.core.correlation import Correlation, current_correlation
