...
from taskport_servicebus import ServiceBusTaskBackend, process_message, receive_forever
...
class TestSending:
    ...
    def test_task_metadata_travels_as_application_properties(self) -> None:
        ...
        assert properties["taskport-task"] == "myapp:reindex"
        assert properties["taskport-queue"] == "metadata"
...
class TestConsumer:
    ...
    def test_the_sdk_generator_body_shape_is_accepted(self) -> None:
        ...
        registry = FunctionRegistry()
        registry.register(lambda: "ok", name="tests:ok")
        raw = json.dumps({"taskport": ENVELOPE_VERSION, "task": "tests:ok"}).encode()
    ...
    def test_receive_forever_completes_on_success(self) -> None:
        registry = FunctionRegistry()
        registry.register(lambda: "ok", name="tests:ok")
        body = json.dumps({"taskport": ENVELOPE_VERSION, "task": "tests:ok"}).encode()
    ...
    def test_receive_forever_abandons_a_failure_for_redelivery(self) -> None:
        body = json.dumps({"taskport": ENVELOPE_VERSION, "task": "tests:missing"}).encode()
        client = FakeServiceBus(messages=[SimpleNamespace(message_id="m1", body=body)])
        calls = {"n": 0}
