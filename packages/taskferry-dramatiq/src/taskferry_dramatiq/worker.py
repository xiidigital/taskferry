"""The worker side — one Dramatiq actor that runs every Taskferry task.

```mermaid
sequenceDiagram
    participant B as Redis / RabbitMQ
    participant W as dramatiq worker
    participant A as taskferry_execute actor
...
the actor and Dramatiq applies its own exponential backoff with jitter; Taskferry
...
:class:`~taskferry.functions.FunctionRegistry` with an allowlist when anything
...
from taskferry.envelope import Envelope, execute_envelope
from taskferry.functions import FunctionRegistry

DEFAULT_ACTOR_NAME = "taskferry_execute"


def dispatch(envelope: Envelope, *, registry: FunctionRegistry | None = None) -> Any:
    """Run one Taskferry envelope. Independent of Dramatiq, so it unit-tests as a dict."""
...
def build_dispatch_actor(
    ...
    **actor_options: Any,
    ...
    """Build and return the Dramatiq actor that runs Taskferry tasks.
    ...
    def taskferry_execute(envelope: Envelope) -> Any:
        return dispatch(envelope, registry=registry)

    return taskferry_execute
