...
The two worker-based engines Taskferry adapts, side by side — and the differences
...
middleware Taskferry cannot see from here — and a capability that is true only
...
class DramatiqTaskBackend(BaseBackend):
    """Sends Taskferry tasks as Dramatiq messages.

    Args:
        actor: The dispatch actor built by
            :func:`~taskferry_dramatiq.build_dispatch_actor`, or a
    ...
    def actor(self) -> Any:
        ...
        if self._actor is None:
            raise ConfigurationError(
                ...
                "actor='myapp.worker:taskferry_execute', or broker=<broker>. "
                "Build it with taskferry_dramatiq.build_dispatch_actor()."
            )
